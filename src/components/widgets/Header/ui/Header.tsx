            ...
            >
              {headerElements.map((element) => (
                <NavigationMenuItem key={element.href}>
                  <NavigationMenuTrigger
                    className={cn(
                      'relative px-4 py-2 text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-100 transition-all duration-200 rounded-lg hover:bg-white/60 group',
                      isActive(element.href) &&
                        'text-indigo-600 dark:text-indigo-600 dark:hover:text-indigo-800 bg-indigo-50 font-medium shadow-sm'
                    )}
                  >
