...
const EditableField: FC<EditableFieldProps> = memo(
  ({ field, currentValue, row, onChange }) => {
    ...
    const handleDateChange = useCallback(
      (date: CalendarDate | null, timeValue: Time | null) => {
        if (!editable) return;
        if (date) {
          try {
            ...
            handleFieldChange(
              format(updatedDateTime, "yyyy-MM-dd'T'HH:mm:ss.SSSxxx")
            );
          } catch {
            handleFieldChange('');
          }
    ...
    const handleTimeChange = useCallback(
      (time: Time | null, dateValue: CalendarDate | null) => {
        if (!editable) return;
        if (time && dateValue) {
          try {
            const newDateTime = dateValue.toDate(getLocalTimeZone());
            const updatedDateTime = setHours(
              setMinutes(setSeconds(newDateTime, time.second), time.minute),
              time.hour
            );
            handleFieldChange(
              format(updatedDateTime, "yyyy-MM-dd'T'HH:mm:ss.SSSxxx")
    ...
    try {
        ...
        return (
          <div key={key}>
            <Text
              ...
              mb="1"
              weight="bold"
              className={cn(!editable && 'text-gray-500')}
            >
              {label}
            </Text>
            <SelectRoot
              ...
              onValueChange={(value) =>
                editable && handleFieldChange(value === 'true')
              }
              disabled={!editable}
            >
              <Trigger
      ...
      if (fieldType === 'date') {
        ...
        const dateValue = dateString
          ? parseDate(dateString.split('T')[0])
          : null;
      ...
      return (
        <div key={key}>
          ...
          <TextField.Root
            ...
            placeholder={
              editable ? `Enter ${label.toLowerCase()}` : 'Read only'
            }
            disabled={!editable}
      ...
      console.error(error, { field, currentValue, row });
      return (
        <div key={key}>
          <Text as="div" size="2" mb="1" weight="bold" className="text-red-500">
            {label} (Error)
          </Text>
          <div className="p-2 bg-red-50 text-red-700 rounded border">
